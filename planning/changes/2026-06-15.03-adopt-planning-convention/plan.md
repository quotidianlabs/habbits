...
**Architecture:** Pure documentation reorganization — create scaffolding files, `git mv` existing specs/plans into `planning/changes/<bundle>/{design,plan}.md`, prepend frontmatter, build a README Index. The 115-test suite must stay green throughout (proof that no code moved).
...
**Files:**
- Move: each `docs/superpowers/specs/<spec>.md` → `planning/changes/<bundle>/design.md`
- Move: each `docs/superpowers/plans/<plan>.md` → `planning/changes/<bundle>/plan.md`
...
for d in planning/changes/*/; do echo "$d -> $(ls "$d" | tr '\n' ' ')"; head -1 "$d/design.md"; head -1 "$d/plan.md"; done
```
...
On merge the folder moves to `changes/` with `status: shipped`, `pr:`,
...
```bash
cd /Users/kevinsmith/src/habbits
grep -oE 'changes/[^)]+design\.md' planning/README.md | while read -r p; do test -f "planning/$p" && echo "ok $p" || echo "BROKEN $p"; done
```
...
echo "every archived bundle frontmatter:"; grep -L '^---' planning/changes/*/*.md || echo "all have frontmatter ✓"
flutter analyze
flutter test
```
...
- `git mv planning/changes/active/2026-06-15.03-adopt-planning-convention planning/changes/`
