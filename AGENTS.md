...
models — there is no mapper layer, so `lib/domain/` imports the generated database library on
purpose. What actually holds, and what `test/architecture_test.dart` enforces: **`lib/domain/`
takes no direct `package:flutter` or `package:drift` import, its only `lib/data/` import is the
generated database library, and nothing under `lib/domain/` or `lib/data/` imports `lib/ui/`.**
`lib/main.dart` is the composition root and sits outside the rule.
[`docs/adr/0001-drift-rows-are-the-domain-model.md`](docs/adr/0001-drift-rows-are-the-domain-model.md)
is why, and is worth reading before "fixing" that import.
...
- Views `ref.watch` a view model and call its command methods. Shared widgets in
  `lib/ui/widgets/` are pure — they take values and return user input, and the screen acts on
  it. Feature-local widgets under `lib/ui/<feature>/widgets/` may read providers.
